...
export class ApiParser {
  ...
  async parseAllPages(searchUrl: string): Promise<ParseResult> {
    ...
    try {
      ...
      console.log(`🔍 Raw API Response:`, JSON.stringify(firstResponse, null, 2));
  ...
  private extractPrice(priceString: string | number): number {
    if (!priceString) return 0;
    
    // If it's already a number
    if (typeof priceString === 'number') return priceString;
    
    const numbers = priceString.toString().match(/[\d\s,]+/g);
    if (numbers) {
  ...
  private extractCurrency(priceString: string | number): string {
    ...
    if (priceStr.includes('лв') || priceStr.includes('BGN')) return 'BGN';
